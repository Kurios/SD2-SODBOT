...
var sqlHelper_1 = require("../general/sqlHelper");
...
var Replays = /** @class */ (function () {
    ...
    Replays.extractReplayInfo = function (message, perms) {
        ...
        ax.get(url).then(function (res) { return __awaiter(_this, void 0, void 0, function () {
            var g, replayPlayer, uuid, winners, loosers, winnerList, looserList, ratings, channel, _i, _a, player, playerid, replayPlayer_1, replayPlayer_2, replayPlayer_3, p1Elo, p2Elo, _b, _c, player, playerid, replayPlayer_4, replayPlayer_5, replayPlayer_6, p1Elo, p2Elo, p1Elo, p2Elo, embed, _d, _e, player, playerid, discordId, elo, discordUser, user, raitingsString, elox, counter, _f, _g, player, playerid, discordId, discordUser, user, elo, elox;
            return __generator(this, function (_h) {
                switch (_h.label) {
                    case 0:
                        g = gameParser_1.GameParser.parseRaw(res.data);
                        replayPlayer = g.players[g.ingamePlayerId];
                        return [4 /*yield*/, sqlHelper_1.SqlHelper.setReplay(message, g)];
                    case 1:
                        uuid = _h.sent();
                    ...
                    case 2:
                        ...
                        if (!(g.players.length == 2 && uuid.rows.length == 0 && perms.isEloComputed && g.version >= 51345)) return [3 /*break*/, 7];
                        return [4 /*yield*/, sqlHelper_1.SqlHelper.getElos(winnerList[0].id, message.channel.id, message.guild.id)];
                    case 3:
                        p1Elo = _h.sent();
                        return [4 /*yield*/, sqlHelper_1.SqlHelper.getElos(looserList[0].id, message.channel.id, message.guild.id)];
                    case 4:
                        p2Elo = _h.sent();
                        ratings = rating_1.RatingEngine.rateMatch(p1Elo, p2Elo, 1);
                        return [4 /*yield*/, sqlHelper_1.SqlHelper.setElos(ratings.p1, { impliedName: winnerList[0].name, serverName: message.guild.name, channelName: channel.name })];
                    case 5:
                        _h.sent();
                        return [4 /*yield*/, sqlHelper_1.SqlHelper.setElos(ratings.p2, { impliedName: looserList[0].name, serverName: message.guild.name, channelName: channel.name })];
                    case 6:
                        _h.sent();
                    ...
                    case 8:
                        ...
                        if (!(g.players.length == 2 && uuid.rows.length == 0 && perms.isEloComputed && g.version >= 51345)) return [3 /*break*/, 13];
                        return [4 /*yield*/, sqlHelper_1.SqlHelper.getElos(winnerList[0].id, message.channel.id, message.guild.id)];
                    case 9:
                        p1Elo = _h.sent();
                        return [4 /*yield*/, sqlHelper_1.SqlHelper.getElos(looserList[0].id, message.channel.id, message.guild.id)];
                    ...
                    case 14:
                        winners = "no one";
                        loosers = "everyone";
                        if (!(g.players.length == 2 && uuid.rows.length == 0 && perms.isEloComputed && g.version >= 51345)) return [3 /*break*/, 19];
                        return [4 /*yield*/, sqlHelper_1.SqlHelper.getElos(g.players[0].id, message.channel.id, message.guild.id)];
                    case 15:
                        p1Elo = _h.sent();
                        return [4 /*yield*/, sqlHelper_1.SqlHelper.getElos(g.players[1].id, message.channel.id, message.guild.id)];
                    case 16:
                        p2Elo = _h.sent();
                        ratings = rating_1.RatingEngine.rateMatch(p1Elo, p2Elo, .5);
                        return [4 /*yield*/, sqlHelper_1.SqlHelper.setElos(ratings.p1, { impliedName: g.players[0].name, serverName: message.guild.name, channelName: channel.name })];
                    case 17:
                        _h.sent();
                        return [4 /*yield*/, sqlHelper_1.SqlHelper.setElos(ratings.p2, { impliedName: g.players[1].name, serverName: message.guild.name, channelName: channel.name })];
                    case 18:
                        _h.sent();
                    ...
                    case 20:
                        ...
                        discordId = "";
                        elo = "";
                        return [4 /*yield*/, sqlHelper_1.SqlHelper.getDiscordUserFromEugenId(player.id)];
                    case 21:
                        discordUser = _h.sent();
                        if (discordUser)
                    ...
                    case 24:
                        ...
                        if (!(g.players.length == 2 && uuid.rows.length == 0 && perms.isEloComputed && g.version >= 51345)) return [3 /*break*/, 25];
                        ...
                        return [3 /*break*/, 27];
                    case 25: return [4 /*yield*/, sqlHelper_1.SqlHelper.getElos(player.id, message.channel.id, message.guild.id)];
                    case 26:
                        elox = _h.sent();
                        if (perms.isChannelEloShown) {
                    ...
                    case 34:
                        elo = "";
                        return [4 /*yield*/, sqlHelper_1.SqlHelper.getElos(player.id, message.channel.id, message.guild.id)];
                    case 35:
                        elox = _h.sent();
                        if (perms.isChannelEloShown) {
                            elo += "Channel ELO: " + Math.round(elox.channelElo);
